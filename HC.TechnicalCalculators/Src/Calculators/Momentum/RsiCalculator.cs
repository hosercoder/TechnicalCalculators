...
namespace HC.TechnicalCalculators.Src.Calculators.Momentum
{
    ...
    /// </summary>
    public class RsiCalculator : BaseCalculator
    {
        private const int MIN_PERIOD = 2;
        private const int MAX_PERIOD = 100;
        private const int DEFAULT_PERIOD = 14;

        public RsiCalculator(Dictionary<string, string> para, string name) : base(name, para)
        {
            ValidateParameters();
        }
        ...
        protected override CalculatorResults CalculateInternal(double[,] prices)
        {
            ...
            var retCode = Core.Rsi(Close, 0, Close.Length - 1, rsi, out outBegIdx, out outNbElement, period);
            ValidateTALibResult(retCode, nameof(CalculatorNameEnum.RSI));
            ...
            for (int i = outBegIdx; i < outBegIdx + outNbElement; i++)
            {
                ...
                {
                    new KeyValuePair<string, double>(nameof(TechnicalNamesEnum.RSI), rsi[i - outBegIdx])
                };
            }
        ...
        {
            return new string[] { nameof(TechnicalNamesEnum.RSI) };
        }
