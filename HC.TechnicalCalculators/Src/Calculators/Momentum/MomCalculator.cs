...
{
    public class MomCalculator : BaseCalculator
    {
        private const int MIN_PERIOD = 1;
        private const int MAX_PERIOD = 100;
        private const int DEFAULT_PERIOD = 10;

        public MomCalculator(Dictionary<string, string> para, string name) : base(name, para)
        {
            ValidateParameters();
        }
        ...
        protected override CalculatorResults CalculateInternal(double[,] prices)
        {
            ...
            var mom = CalculateMOM(Close, period);
            ...
            for (int i = 0; i < mom.Length; i++)
            {
                ...
                {
                    new KeyValuePair<string, double>(nameof(TechnicalNamesEnum.MOM), mom[i])
                };
            }
        ...
        {
            return new string[] { nameof(TechnicalNamesEnum.MOM) };
        }
        ...
        private double[] CalculateMOM(double[] prices, int period)
        {
            int outBegIdx, outNbElement;
            double[] mom = new double[prices.Length];
            Core.Mom(prices, 0, prices.Length - 1, mom, out outBegIdx, out outNbElement, period);
            return mom;
        }
    }
}
