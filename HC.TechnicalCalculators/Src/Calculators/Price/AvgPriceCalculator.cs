...
namespace HC.TechnicalCalculators.Src.Calculators.Price
{
    ...
    /// </summary>
    public class AvgPriceCalculator : BaseCalculator
    {
        public AvgPriceCalculator(Dictionary<string, string> para, string name) : base(name, para)
        {

        }
        protected override CalculatorResults CalculateInternal(double[,] prices)
        {
            var avgPrice = CalculateAVGPRICE(Open, High, Low, Close);
            ...
            for (int i = 0; i < avgPrice.Length; i++)
            {
                ...
                {
                    new KeyValuePair<string, double>(nameof(TechnicalNamesEnum.AVGPRICE), avgPrice[i])
                };
            }
        ...
        {
            return new string[] { nameof(TechnicalNamesEnum.AVGPRICE) };
        }
        ...
        private double[] CalculateAVGPRICE(double[] open, double[] high, double[] low, double[] close)
        {
            int outBegIdx, outNbElement;
            double[] avgPrice = new double[open.Length];
            Core.AvgPrice(open, high, low, close, 0, open.Length - 1, avgPrice, out outBegIdx, out outNbElement);
            return avgPrice;
        }
    }
}
