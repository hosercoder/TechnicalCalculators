...
namespace HC.TechnicalCalculators.Src.Calculators.Overlap
{
    ...
    /// </summary>
    public class ExponentialMovingAverage : BaseCalculator
    {
        private const int MIN_PERIOD = 2;
        private const int MAX_PERIOD = 200;
        private const int DEFAULT_PERIOD = 20;

        public ExponentialMovingAverage(Dictionary<string, string> para, string name) : base(name, para)
        {
            ValidateParameters();
        }
        ...
        protected override CalculatorResults CalculateInternal(double[,] prices)
        {
            ...
            var retCode = Core.Ema(Close, 0, Close.Length - 1, movingAvg, out outBegIdx, out outNbElement, period);
            ValidateTALibResult(retCode, nameof(CalculatorNameEnum.EMA));
