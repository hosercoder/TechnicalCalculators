...
{
    public class MovingAverageCalculatorTests
    {
        ...
        public void Constructor_ShouldThrowArgumentException_WhenPeriodParameterIsMissing()
        {
            ...
            // Act & Assert
            var exception = Assert.Throws<ArgumentException>(() => new MovingAverageCalculator(parameters, nameof(CalculatorNameEnum.MA)));
        ...
        public void Constructor_ShouldCreateInstance_WhenPeriodParameterIsPresent()
        {
            ...
            // Act
            var calculator = new MovingAverageCalculator(parameters, nameof(CalculatorNameEnum.MA));
        ...
        public void Calculate_ShouldReturnCorrectResults_WhenParametersAreValid()
        {
            ...
            Assert.Equal(nameof(CalculatorNameEnum.MA), result.Name);
            Assert.True(result.Results.Count > 0);

            // Check that number of results is correct: prices.Length - period + 1
            Assert.Equal(prices.GetLength(0) - 5 + 1, result.Results.Count);
        ...
        [Fact]
        public void Calculate_ShouldComputeCorrectMovingAverageValues()
        {
            // Arrange
            ...
            Assert.Equal(20, result.Results[1002][0].Value);
            Assert.Equal(30, result.Results[1003][0].Value);
            Assert.Equal(40, result.Results[1004][0].Value);
        }
        ...
        public void Calculate_DifferentPeriods_ShouldProduceDifferentResults()
        {
            ...
            var calculator1 = new MovingAverageCalculator(parameters1, nameof(CalculatorNameEnum.MA));
            var calculator2 = new MovingAverageCalculator(parameters2, nameof(CalculatorNameEnum.MA));
            ...
            if (commonTimestamps.Any())
            {
                ...
                var ma1 = result1.Results[commonTimestamp][0].Value;
                var ma2 = result2.Results[commonTimestamp][0].Value;

                // Different periods should produce different MA values
        ...
        public void Calculate_ShouldProduceValuesWithinRangeOfInputPrices()
        {
            ...
            // Assert
            // Each MA value should be within the range of input prices
            foreach (var value in result.Results.Values)
            {
                double ma = value[0].Value;
                Assert.InRange(ma, minClose, maxClose);
            }
        }

        [Fact]
        public void Calculate_WithConstantPrices_ShouldReturnConstantMA()
        {
            // Arrange
            var parameters = new Dictionary<string, string> { { nameof(ParameterNamesEnum.Period), "3" } };
            var calculator = new MovingAverageCalculator(parameters, nameof(CalculatorNameEnum.MA));
            ...
            // Assert
            // All MA values should equal the constant price
            foreach (var value in result.Results.Values)
            {
