...
namespace HC.TechnicalCalculators.Tests.Calculators.Momentum
{
    public class RocCalculatorTests
    {
        ...
        public void Constructor_ShouldSetDefaultPeriod_WhenPeriodParameterIsMissing()
        {
            ...
            // Act
            var calculator = new RocCalculator(parameters, nameof(CalculatorNameEnum.ROC));
            ...
            Assert.Equal(nameof(CalculatorNameEnum.ROC), results.Name);
        ...
        public void Calculate_ShouldThrowFormatException_WhenPeriodIsNotAnInteger()
        {
            ...
            // Act & Assert
            var exception = Assert.Throws<ArgumentException>(() => new RocCalculator(parameters, nameof(CalculatorNameEnum.ROC)));
        ...
        public void Calculate_ShouldReturnValidResults_WithTypicalParameters()
        {
            ...
            {
                Assert.Single(result.Value); // Should have one ROC value
                Assert.Equal(nameof(TechnicalNamesEnum.ROC), result.Value[0].Key);

                // ROC can be positive or negative, so no specific range check
            }
        }
        ...
        [Fact]
        public void Calculate_WithUptrendData_ShouldShowPositiveROC()
        {
            // Arrange
            var parameters = new Dictionary<string, string>
            {
            ...
            // Skip initial values where ROC is still forming
            ...
            // Calculate how many ROC values are positive
            int positiveCount = 0;
            ...
            // In an uptrend, most ROC values should be positive
            Assert.True(positiveCount > laterValues.Count / 2,
                $"Expected majority of ROC values to be positive in an uptrend, but only {positiveCount} out of {laterValues.Count} were positive");
        }

        [Fact]
        public void Calculate_WithDowntrendData_ShouldShowNegativeROC()
        {
            // Arrange
            var parameters = new Dictionary<string, string>
            {
            ...
            // Calculate how many ROC values are negative
            int negativeCount = 0;
            foreach (var result in laterValues)
            {
                if (result.Value[0].Value < 0)
            ...
            // In a downtrend, most ROC values should be negative
            Assert.True(negativeCount > laterValues.Count / 2,
                $"Expected majority of ROC values to be negative in a downtrend, but only {negativeCount} out of {laterValues.Count} were negative");
        }

        [Fact]
        public void Calculate_WithConstantPriceData_ShouldShowZeroROC()
        {
            // Arrange
            var parameters = new Dictionary<string, string>
            {
            ...
            // With constant prices, ROC should be zero after the period
            foreach (var result in laterValues)
            {
                Assert.Equal(0, result.Value[0].Value);
            }
        }
        ...
        private double[,] CreateConstantPriceData(int length)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                prices[i, 2] = basePrice + 0.5; // high
                prices[i, 3] = basePrice - 0.5; // low
                prices[i, 4] = basePrice; // close
